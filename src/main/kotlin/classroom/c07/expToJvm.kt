...
sealed class Expr() {
    abstract fun visit(mw : MethodVisitor)
    abstract fun stackUsage() : Int

    class Const(val value : Short) : Expr() {
    ...
    class BinOp(val op : Operation, var l : Expr, var r : Expr) : Expr() {
    ...
    private fun getClassWriter() : ClassWriter {
        ...
        mainMethodVisitor.visitMethodInsn(INVOKESTATIC, className, calcMethodName, "()I", false)
...
fun loadClassAndRun(classByteArray : ByteArray) : Any? {
    ...
    for (method in methods) {
        if (method.name != Expr.calcMethodName) {
            continue
        }
